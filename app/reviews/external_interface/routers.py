...
@router.get(
    "", status_code=status.HTTP_200_OK, response_model=List[GetReviewJsonResponse]
)
@inject
def get_reviews(
    ...
    if not output_dto.status:
        return FailedJsonResponse.build_by_output_dto(output_dto)
    get_reviews_json_response = [
        GetReviewJsonResponse.build_by_output_dto(review) for review in output_dto.items
    ]
...
def update_review(
    ...
    output_dto = review_application_service.update_review(
