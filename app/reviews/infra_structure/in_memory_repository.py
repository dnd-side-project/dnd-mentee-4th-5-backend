...
class InMemoryReviewRepository(ReviewRepository):
    ...
    def find_all_by_user_id(
        ...
        return [
            review
            for review in list(self.review_id_to_review.values())
    ...
    def find_by_drink_id_user_id(self, drink_id: DrinkId, user_id: UserId):
        review_id = self.drink_id_user_id_to_review_id.get(
            (str(drink_id), str(user_id)), None
        )
        if not review_id:
            return None
        else:
    ...
    def add(self, review: Review) -> None:
        ...
        self.drink_id_user_id_to_review_id[
            (str(review.drink_id), str(review.user_id))
        ] = str(review.id)
