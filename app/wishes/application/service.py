...
class WishApplicationService:
    ...
    def find_wishes(
        self, input_dto: FindWishesInputDto
    ) -> Union[FindWishesOutputDto, FailedOutputDto]:
        try:
    ...
    def create_wish(
        ...
        try:
            ...
            if (
                self._wish_repository.find(
                    QueryParam(user_id=str(user_id), drink_id=str(drink_id))
                )
                is not None
            ):
            ...
            return CreateWishOutputDto(
                user_id=str(user_id), drink_id=str(drink_id), created_at=wish.created_at
            )
        except Exception as e:
            return FailedOutputDto.build_system_error(message=str(e))
    ...
    def delete_wish(
        ...
        try:
            user_id = UserId(value=input_dto.user_id)
            drink_id = DrinkId.from_str(input_dto.drink_id)
            wish = self._wish_repository.find(
                QueryParam(user_id=str(user_id), drink_id=str(drink_id))
            )
