...
from wishes.application.dto import (CreateWishInputDto, DeleteWishInputDto,
                                    FindWishesInputDto)
from wishes.application.service import WishApplicationService
from wishes.domain.repository import QueryParam
from wishes.external_interface.json_dtos import (CreateWishJsonResponse,
                                                 GetWishesJsonResponse)
...
@router.get(
    "/drink/", status_code=status.HTTP_200_OK, response_model=GetWishesJsonResponse
)
def get_wishes(
    query_param: QueryParam = Depends(),
    wish_application_service: WishApplicationService = Depends(
        Provide[Container.wish_application_service]
    ),
):
...
@router.post(
    ...
    response_model=CreateWishJsonResponse,
)
@inject
def create_wish(
    drink_id: str,
    ...
    input_dto = CreateWishInputDto(
        user_id=get_token_data_output_dto.user_id, drink_id=drink_id
    )
