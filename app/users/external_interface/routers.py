...
from users.application.dtos import (CreateUserInputDto, DeleteUserInputDto,
                                    FindUserInputDto, UpdateUserInputDto)
...
@router.post(
    "", status_code=status.HTTP_201_CREATED, response_model=CreateUserJsonResponse
)
@inject
def create_user(
...
@router.get(
    "/{user_id}", status_code=status.HTTP_200_OK, response_model=GetUserJsonResponse
)
@inject
def get_user(
    user_id: str,
...
def delete_user(
    ...
    ouput_dto = user_application_service.delete_user(input_dto)
    if not ouput_dto.status:
        return FailedJsonResponse.build_by_output_dto(ouput_dto)
