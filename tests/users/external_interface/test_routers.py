from users.external_interface.json_dto import UpdateUserJsonRequest, CreateUserJsonRequest
...
def test_put_users(client):
    ...
    response = client.put(
        "/users",
        json=UpdateUserJsonRequest(user_name="heumsi", description="시흠입니다.", password="1234").dict(),
    )
    ...
    assert response.json() == {"error_type": "Unauthorized Error", "message": "올바른 access-token이 아닙니다."}
    ...
    response = client.post(
        "/users", json=CreateUserJsonRequest(user_id="heumsi", user_name="heumsi", password="1234").dict()
    )
    assert response.status_code == 201
    ...
    response = client.put(
        ...
        json=UpdateUserJsonRequest(user_name="siheum", description="시흠입니다.", password="1234").dict(),
    )
